...
- **A hosted-LLM config** (OpenAI-compatible API, etc.) — see `docs/llm-providers.md`. The LLM layer is deliberately left at one backend; the backend choice itself is still open.
...
- Does generated-material history matter, or is overwrite-in-place (current behavior) fine? Affects whether `docs/data-model.md`'s proposed `GeneratedMaterial` table is worth building.
